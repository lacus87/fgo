...
Page({
  ...
  getRemoteSetting: function () {
    ...
    var that = this;
    wx.request({
      url: app.globalData.url +'/fgo/servant/syncLocalSetting.do?key=' + key,
      // url: 'http://127.0.0.1/fgo/servant/syncLocalSetting.do?key=' + key,
      success: function (res) {
    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          //本地记录更新
          wx.request({
            url: app.globalData.url +"/fgo/servant/uploadLocalSetting.do?key=" + key,
            // url: "http://127.0.0.1/fgo/servant/uploadLocalSetting.do?key=" + key,
            method: "POST",
            header: {
