...
Page({
  ...
  onLoad: function (options) {
    ...
    setTimeout(function () {
      wx.getSystemInfo({
        success: function (res) {
          that.setData({
            pageHeight: res.windowHeight - 110,
            model: app.globalData.model
          });
        }
      });
    ...
    this.setData({
      servantImg: app.globalData.url + "/images/servant/" + options.id.substring(1, 4) + ".jpg",
    ...
    wx.request({
      url: app.globalData.url + '/servant/getServantMaterialWithGodCloth.do?servantId=' + options.id + "&rank=" + options.rank + "&skill=" + options.skill + "&clothFlag=" + options.clothFlag,
      method: 'GET',
      success: function (res) {
