...
Page({
  ...
  onLoad: function (options) {
    ...
    wx.request({
      url: app.globalData.url + '/material/getMaterialDrop.do?materialId=' + materialId + "&model=" + dropModel,
      method: 'GET',
      success: function (res) {
  ...
  setServantStatus: function () {
    ...
    wx.request({
      url: app.globalData.url +"/material/countServantMaterial.do?materialId=" + materialId,
      method: "POST",
      header: {
  ...
  changeView: function(e){
    var sortType = this.data.sortType ==1? 0:1;
    var dropList = this.data.dropList;
    dropList.sort(function(a,b){
      if(sortType == 1){
        var apA = parseFloat(a.ap.replace("AP/个",""));
        var apB = parseFloat(b.ap.replace("AP/个", ""));
        return apA < apB ? -1 : 1;
      }else{
